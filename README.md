...
- `git+https://` prefix is normalized away (e.g. `git+https://github.com/...` ->
  `https://github.com/...`).
- URLs that already end with known archive extensions (zip/tar.* etc.), archive
  patterns, release assets, raw file URLs, and Bitbucket `get` URLs are returned
  as-is (possibly with credentials removed and host normalized to lowercase).
...
- If you rely on automatic default-branch detection, consider enabling an API
  lookup to avoid incorrect assumptions about `main` vs `master` (disabled by
  default).
